...
**Derivation from normative rule**: Per FAST 1.1 §10.6.1.1 (p.23): "The entity value is a two's complement integer representation [TWOC]. The most significant data bit of the entity value is the sign bit." The stop-bit encoding must ensure the value is not overlong (§10.6.1, p.23). With 9 bytes (63 data bits), sign-extending the 64-bit pattern `0x8000000000000000` to 63 bits yields bit 62 = 1, bits [61..0] = 0, but this represents -4611686018427387904, not INT64_MIN. With 10 bytes (70 data bits), sign-extending yields bits [69..63] = `1111111` = `0x7F` (sign bit = 1 → negative). This is the minimum correct encoding. Overlong check: removing the top 7 data bits yields a 63-bit positive value (sign bit at bit 62 = 0) ≠ INT64_MIN, so the encoding is not overlong.

**Production decoder defect**: `wire_cursor.cpp:178` computes `sign = (raw >> 63) & 1` before the 10th byte, but after accumulating the first 9 bytes the decoder stores 63 data bits in `raw` positions [62..0] — bit 63 is always 0. The normative first wire group (bits [69..63] of the 70-bit entity) is `0x7F`, but `raw >> 57` extracts bits [62..57] = `0x3F`, not `0x7F`. Therefore `sign` is always 0, `expected` is always `0x00`, and the guard at line 181 rejects 10-byte negative values (including INT64_MIN) because `0x3F ≠ 0x00` → IntegerOverflow. This is a **defect** in the production decoder.

**Reference encoder defect**: `fast_reference_encoder.hpp:128-147` copies the 64-bit signed value into `uint64_t raw` via `memcpy` and extracts 7-bit groups via `(raw >> (g * 7)) & 0x7F`. For INT64_MIN (`0x8000000000000000`), group 0 extracts `(raw >> 63) & 0x7F` = `(0x8000000000000000 >> 63) & 0x7F` = `0x01`. The correct normative group 0 requires bits [69..63] = `1111111` = `0x7F` (six sign-extension bits [69..64] plus bit 63). The encoder loses bits [69..64] (all 1 for sign extension) because they don't exist in `uint64_t`, producing `0x01` instead of `0x7F`. This is a **defect** in the reference encoder.
...
**Production decoder defect**: `read_nullable_i32` decodes into `int32_t` (max 2147483647) before applying the nullable inverse mapping. The intermediate value 2^31 overflows `int32_t`. No test exercises the actual nullable signed i32 maximum.
...
| 5 | **Stop-bit signed i64: pre-shift overflow guard rejects INT64_MIN** | `wire_cursor.cpp:177-184`: for bytes beyond 9, checks top 7 bits are sign extension before shifting. `sign = (raw >> 63) & 1`; `top7 = raw >> 57`; `expected = sign ? 0x7F : 0x00` | FAST 1.1: max 10 bytes for int64. The 10th byte's 7 data bits would shift left by 63, overflowing uint64_t. The pre-shift check is a necessary guard. **Defect**: After accumulating the first 9 bytes, `raw` holds 63 data bits in positions [62..0] — bit 63 is always 0. Therefore `sign = (raw >> 63) & 1` is always 0, `expected` is always `0x00`. The normative first wire group for INT64_MIN is `0x7F` (bits [69..63] of the 70-bit entity), but `raw >> 57` extracts bits [62..57] = `0x3F`. The guard rejects INT64_MIN because `0x3F ≠ 0x00` → IntegerOverflow. See §4.29 for literal derivation. | FAST 1.1, p.10 + p.23 | **confirmed discrepancy** (production decoder) |
...
- **#21** (reference encoder): Sign-extension defect for both 32-bit and 64-bit signed values. INT32_MIN produces `[0x08, ...]` instead of `[0x78, ...]`; INT64_MIN produces group 0 = `0x01` instead of `0x7F`. (Consolidates former #29.)
...
9. **INT64_MIN encoding and decoder defect**: The correct encoding is `[0x7F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80]`, derived from the 70-bit two's complement representation (FAST 1.1 §10.6.1.1, p.23). The production decoder's signed i64 pre-shift guard (`wire_cursor.cpp:178`) computes `sign = (raw >> 63) & 1` before the 10th byte, but after 9 bytes `raw` holds 63 data bits in positions [62..0] — bit 63 is always 0. The normative first wire group is `0x7F`, but `raw >> 57` extracts bits [62..57] = `0x3F`; the guard rejects INT64_MIN because `0x3F ≠ 0x00` → IntegerOverflow (see §5 entry #5, §4.29). The reference encoder (`fast_reference_encoder.hpp:128-147`) extracts `(raw >> 63) & 0x7F` = `0x01` for group 0 instead of the correct `0x7F` (bits [69..63] = `1111111`), losing sign-extension bits [69..64] (see §5 entry #21, §4.29).

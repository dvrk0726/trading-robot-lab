...
static void test_size_0() {
    ...
    auto r = frame_udp_message(make_input(data), PreambleByteOrder::LittleEndian, valid_limits(), out);
...
static void test_exact_max() {
    ...
    data[0] = 0x07;
    FramedMessageView out;
    auto r = frame_udp_message(make_input(data), PreambleByteOrder::LittleEndian, limits, out);
    CHECK_EQ(static_cast<int>(r.code), static_cast<int>(FrameCode::Ok));
...
// --- Invalid enum value: InvalidConfig ---
static void test_invalid_enum() {
    auto bad_order = static_cast<PreambleByteOrder>(0xFF);
    std::vector<std::uint8_t> data{0x01, 0x00, 0x00, 0x00, 0x00};
    FramedMessageView out;
    auto r = frame_udp_message(make_input(data), bad_order, valid_limits(), out);
    CHECK_EQ(static_cast<int>(r.code), static_cast<int>(FrameCode::InvalidConfig));
    TEST_PASS("invalid enum -> InvalidConfig");
}

// --- Endian vectors ---
static void test_endian_01_00_00_00() {
    ...
    {
        FramedMessageView out;
        auto r = frame_udp_message(make_input(data), PreambleByteOrder::BigEndian, valid_limits(), out);
        CHECK_EQ(static_cast<int>(r.code), static_cast<int>(FrameCode::Ok));
        CHECK_EQ(out.msg_seq_num, 0x01000000u);
    }
    TEST_PASS("01 00 00 00 endian vectors");
}

static void test_endian_00_00_00_01() {
    ...
    TEST_PASS("00 00 00 01 endian vectors");
}

static void test_endian_01_02_03_04() {
    ...
    }
    {
        ...
        auto r = frame_udp_message(make_input(data), PreambleByteOrder::BigEndian, valid_limits(), out);
        CHECK_EQ(static_cast<int>(r.code), static_cast<int>(FrameCode::Ok));
        CHECK_EQ(out.msg_seq_num, 0x01020304u);
    }
    TEST_PASS("01 02 03 04 endian vectors");
}

static void test_endian_FF_FF_FF_FF() {
    std::vector<std::uint8_t> data{0xFF, 0xFF, 0xFF, 0xFF, 0xAB};
    FramedMessageView out;
    auto r = frame_udp_message(make_input(data), PreambleByteOrder::LittleEndian, valid_limits(), out);
    CHECK_EQ(static_cast<int>(r.code), static_cast<int>(FrameCode::Ok));
    CHECK_EQ(out.msg_seq_num, 0xFFFFFFFFu);
    TEST_PASS("FF FF FF FF -> UINT32_MAX");
}
...
static void test_metadata_propagation() {
    ...
    auto r = frame_udp_message(
        ...
        PreambleByteOrder::LittleEndian, valid_limits(), out);
    CHECK_EQ(static_cast<int>(r.code), static_cast<int>(FrameCode::Ok));
...
static void test_source_immutability() {
    ...
    FramedMessageView out;
    frame_udp_message(make_input(data), PreambleByteOrder::LittleEndian, valid_limits(), out);
    CHECK(data == original);
...
static void test_clear_after_failure() {
    ...
    auto r1 = frame_udp_message(make_input(good), PreambleByteOrder::LittleEndian, valid_limits(), out);
    CHECK_EQ(static_cast<int>(r1.code), static_cast<int>(FrameCode::Ok));
    CHECK_EQ(out.msg_seq_num, 0x42u);
    CHECK_EQ(out.fast_body.size(), 1u);

    auto r2 = frame_udp_message(make_input(bad), PreambleByteOrder::LittleEndian, valid_limits(), out);
...
int main() {
    ...
    test_invalid_enum();
    test_endian_01_00_00_00();
    test_endian_00_00_00_01();
    test_endian_01_02_03_04();
    test_endian_FF_FF_FF_FF();
    test_body_offset();

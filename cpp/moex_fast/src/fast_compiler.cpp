...
OpInstruction parse_operator(pugi::xml_node field_node, DecWireType wire_type,
    ...
    for (auto child : field_node.children()) {
        ...
        // Excluded operators: reject immediately with stable issue code
        if (name == "default" || name == "copy" || name == "increment" ||
            name == "delta" || name == "tail") {
            ctx.error("unsupported_operator",
                      "Unsupported operator <" + name + "> in " + field_path +
                      ": not in accepted MOEX SPECTRA profile", field_path);
            return op;
        }
        ...
        if (name == "constant") {
            ...
            ctx.error("unsupported_operator",
                      "Unsupported operator <" + name + "> in " + field_path, field_path);
        }

...
def render_quality():
    records = fetch_all("SELECT * FROM qsh_quality ORDER BY id")
    if not records:
        rows = '<tr><td colspan="9" style="text-align:center;color:var(--text-muted);padding:20px">No QSH quality reports yet. Run qsh-ingest to generate.</td></tr>'
    else:
        rows = ""
        for r in records:
            rows += f"""<tr>
  <td><code>{esc(r['file_name'])}</code></td>
  <td>{esc(r['stream_type'])}</td>
  <td>{esc(r['instrument'])}</td>
  <td>{r['records_total']:,}</td>
  ...
  <td>{status_badge(r['status'])}</td>
    ...
    return f"""
<h2 class="section-title">Data Quality — C++ QSH Ingest</h2>
<p style="color:var(--text-secondary);margin-bottom:16px;font-size:12px">
  Quality reports generated by <code>qsh-ingest quality</code>.
  C++ engine parses QSH v4 files, scans OrdLog records, and produces Data Quality JSON.
  Run: <code>qsh-ingest quality &lt;file.qsh&gt;</code>
</p>
<div class="pipeline-flow">
  <span class="pipeline-step">QSH File</span><span class="pipeline-arrow">-&gt;</span>
  <span class="pipeline-step">C++ qsh-ingest</span><span class="pipeline-arrow">-&gt;</span>
  <span class="pipeline-step">OrdLog Parser</span><span class="pipeline-arrow">-&gt;</span>
  <span class="pipeline-step">L3 Book</span><span class="pipeline-arrow">-&gt;</span>
  <span class="pipeline-step">L2 Snapshots</span><span class="pipeline-arrow">-&gt;</span>
  <span class="pipeline-step">Data Quality JSON</span>
</div>
<table>
<thead><tr>
  <th>File</th><th>Stream</th><th>Instrument</th>
  <th>Total</th><th>Valid</th><th>Rejected</th>
  <th>Transactions</th><th>Book Errors</th><th>Status</th>
</tr></thead>
<tbody>{rows}</tbody>
</table>
"""
